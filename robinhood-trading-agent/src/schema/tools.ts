...
export const checkSafetyInputSchema = z.object({
  ...
  marginMaintenanceUtilization: z.number().finite().min(0).max(1).nullable().default(null),
});
...
export const sizeOrderInputSchema = z.object({
  ...
  contributingSignals: z.array(z.object({ key: z.enum(SIGNAL_KEYS), vote: z.number().finite().min(-1).max(1), weight: z.number().finite().min(0), detail: z.string() })),
  cash: z.number().finite().min(0),
  maxMarginBuyingPower: z.number().finite().min(0).default(0),
...
export const recordOutcomeInputSchema = z.object({
  ...
  realizedReturnPct: z.number().finite(),
